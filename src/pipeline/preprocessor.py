...
def preprocess(data):
    ...
    Args:
        data (str): The raw chat data string.

    Returns:
        pd.DataFrame: A DataFrame with columns for user, message, date, and derived time features.
    """
    try:
        logging.info("Starting preprocessing.")

        # Define flexible patterns to handle various WhatsApp timestamp formats
        pattern_12hr = r'\d{1,2}[/.-]\d{1,2}[/.-]\d{2,4},\s\d{1,2}:\d{2}\s?[APapm]{2}\s*-\s*'  # Handles M/D/YYYY, h:mm AM/PM
        pattern_24hr = r'\d{1,2}[/.-]\d{1,2}[/.-]\d{2,4},\s\d{1,2}:\d{2}\s*-\s*'             # Handles M/D/YYYY, HH:mm

        # Try to split using the 12-hour pattern first
        messages_12hr = re.split(pattern_12hr, data)[1:]
        dates_12hr = re.findall(pattern_12hr, data)

        # If no matches found, try the 24-hour pattern
        if not dates_12hr:
            messages = re.split(pattern_24hr, data)[1:]
            dates = re.findall(pattern_24hr, data)
            time_format = '24hr'
        else:
            messages = messages_12hr
            dates = dates_12hr
            time_format = '12hr'
        ...
        # Clean up the extracted date strings
        cleaned_dates = [d.replace('\u202f', ' ').strip() for d in dates]
        cleaned_dates = [re.sub(r'(\d)\s*([APMapm]{2})', r'\1 \2', d) for d in cleaned_dates]

        df = pd.DataFrame({'user_message': messages, 'message_date': cleaned_dates})

        # Parse dates based on detected format
        if time_format == '12hr':
            df['message_date'] = pd.to_datetime(df['message_date'], format='%m/%d/%y, %I:%M %p', errors='coerce')
        else:  # 24hr
        ...
        num_parsed = df['message_date'].notna().sum()
        num_failed = df['message_date'].isna().sum()
